...
export const Ranking = ({ jogador, ranking, prog, type, onChangeType, onBack, licao, rankingLoading, onRefresh }: any) => {
  ...
  // pode abrir a própria linha; admin abre qualquer aluno; professor só abre
  // alunos do local atribuído a ele em teacherAssignments. Não se aplica às
  // linhas de DUPLA (isPair) — cada uma representa duas pessoas, e o "dia a
  // dia" de uma dupla já tem tela própria (aba Dupla).
  ...
  const canExpand = (r: any) => !isPair && (r.id === jogador.id || !!jogador.isAdmin || allowedForProf.has(r.id));
  ...
  // total do ranking — só que aqui detalhado por dia (history[diaId].xp) em
  // vez de somado. Para o próprio usuário usa `prog` (estado mais fresco); na
  // aba Semana os outros já vêm com `history` embutido (doc cru, via
  // weekRows); nas abas de Campanha (soma agregada, sem history) busca sob
  // demanda ao expandir.
  ...
  const renderDayDetail = (r: any) => {
    ...
    return (
      <div style={{padding:'0 16px 14px',borderTop:'1px solid var(--b2)'}}>
        <div style={{fontSize:11,color:'var(--mut)',margin:'12px 0 8px',letterSpacing:'.04em'}}>PONTUAÇÃO POR DIA · {(licao?.titulo || 'ESTA SEMANA').toUpperCase()}</div>
